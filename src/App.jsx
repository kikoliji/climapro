          ...
          <div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:6}}>
              {nombresActivos.map(nom=>{
                ...
                return <button key={nom} type="button" onClick={()=>toggleAsignado(nom)} style={{padding:"6px 14px",borderRadius:20,cursor:"pointer",border:"none",fontFamily:"Inter",fontSize:12,fontWeight:500,background:sel?COLORS.accent:COLORS.surface,color:sel?"#000":COLORS.muted,outline:sel?`2px solid ${COLORS.accent}`:`1px solid ${COLORS.border}`}}>{sel?"✓ ":""}{nom}</button>;
              })}
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
